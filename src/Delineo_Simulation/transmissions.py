...
# user enters a number for days of incubaton, from 3-14
incubation = int(
    input("Enter average number of days of incubation (from 3 - 14): "))
...
# user enters a number for days of infection, from 14-28
infection = int(
    input("Enter average number of days of infection (from 14 - 28): "))
while (infection < 14 or infection > 28):
    print("Infection out of range, please try again.")
    infection = int(
        input("Enter average number of days of infection (from 14 - 28): "))

# time infectious is the average time it takes for exposed->infectious (15 days * 24 hours/day)
timeInfectious = infection*24
...
cleanlinessMult = float(
    input("Enter cleanliness multiplier (from 0.05 - 0.15): "))
...
if (facility == "Hospital"):
    hospitalDeathRate = float(
        input("Enter hospital death rate (0.01 - 0.02): "))
    while (hospitalDeathRate < 0.01 or hospitalDeathRate > 0.02):
        print("Hospital death rate out of range, please try again.")
        hospitalDeathRate = float(
            input("Enter hospital death rate (from 0.01 - 0.02): "))

if (facility == "Nursing home"):
    nursingDeathRate = float(
        input("Enter nursing home death rate (0.05 - 0.1): "))
...
if (facility == "Prison"):
    ...
    while (prisonDeathRate < 0.05 or prisonDeathRate > 0.2):
        ...
        prisonDeathRate = float(
            input("Enter prison death rate (0.05 - 0.2): "))
...
def main():
    ...
    """
    print("Average change in susceptible individuals: " +
          str(hours*changeSusceptible(s0, e0, i0, r0)))
    print("Average change in exposed individuals: " +
          str(hours*changeExposed(s0, e0, i0, r0)))

    # Specific information available in ICU, Hospitals, prisons, nursing homes
    if (facility == "ICU" or facility == "Hospital" or facility == "Nursing home" or facility == "Prison"):
        print("Average change in infected individuals: " +
              str(hours*changeInfected(s0, e0, i0, r0)))
        print("Average change in recovered individuals: " +
              str(hours*changeRecovered(s0, e0, i0, r0)))
        print("Average change in dead individuals: " +
              str(hours*changeDead(s0, e0, i0, r0)))
    ...
    # prints the transmission rate for the values of current s0, e0, i0, and r0
...
def transmissionRate(s0, e0, i0, r0):
    ...
    """
    n = s0 + e0 + i0 + \
        r0  # n is the total number of people (equivalent to variable "total" earlier in the script)
    ...
    beta = densityMult * density + contactMult * contact + \
        cleanlinessMult * cleanliness + mobilityMult * mobility
    return beta
...
def changeSusceptible(s0, e0, i0, r0):
    ...
    ds_dt = -1*(overallContagiousness() *
                transmissionRate(s0, e0, i0, r0)*s0*i0)/total
    return ds_dt
...
def changeDead(s0, e0, i0, r0):
    ...
    if (facilityType == 'Hospital'):
        ...
        dd_dt = dd_dt = i0*(overallSeverityRisk() *
                            prisonDeathRate)/timeInfectious
    else:
        dd_dt = dd_dt = 0
    return dd_dt
